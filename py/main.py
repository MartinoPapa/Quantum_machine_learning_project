...
### Logger class to print in both console and output file
class Logger(object):
...
THETA_0_INIT = 1.0 #probabilities of Kraus matrixes          
THETA_1_INIT = -5.0   
...
# DATASET SAMPLING WITH A BFS STARTING FROM THE HUBS
class PaySimDataset:
    def __init__(self, csv_file, pool_file="pool_nodes.pkl"):
        ...
        if os.path.exists(pool_file):
            print(f"Loading from {pool_file}")
            with open(pool_file, "rb") as f:
                self.allowed_nodes = pickle.load(f)
        else:
            raise FileNotFoundError(f"Please run 'find_pool_of_components.py' to generate {pool_file}")
    ...
    def _sample_subgraphs_bfs_balanced(self):
        ...
        normal_starts.sort(key=lambda n: degrees[n], reverse=True)
...
### Memory Bank used to implement S_normal during the training
class MemoryBank:
